...
| 技能库 | 公共技能 / 技能市场 | 管理平台公共技能，浏览并同步技能市场 | 发布、编辑、取消公开、导出、同步 |
...
4. `生成应用`用于从当前工作台技能生成应用。
5. `同步`仅对已公开技能显示，点击后先弹确认弹窗；用户确认后，将当前工作台版本同步到公共技能。
6. `导出`导出当前技能 JSON。
7. `删除`删除当前工作台中的该技能。
8. 点击`使用`后，将该技能带入当前对话使用链路。
9. 当用户在`基本信息弹窗`或`配置弹窗`中保存已公开技能后，系统先保存当前工作台修改，再弹出“是否同步公共技能”的确认弹窗；无论用户选择`同步`还是`暂不同步`，当前工作台保存结果都保持生效，只有确认同步时才同步公共技能。
...
4. 点击`取消公开`时，二次确认后将该技能从公共技能列表撤下。
5. 点击`导出`时，导出该技能 JSON。
6. 公共技能不单独提供`删除`动作；`取消公开`即表示从公共技能列表移除。
...
9. 技能库中的`发布`进入发布确认弹窗；技能库公共技能不单独提供`更新`动作。
10. 工作台中的已公开技能在基本信息或配置保存后，需要追加一次“是否同步公共技能”的确认弹窗。
...
flowchart TD
  ...
  I -- "否" --> J["保留为当前工作台技能"]
  I -- "是" --> K["弹窗询问是否同步公共技能"]
  K -- "同步" --> L["保存结果保持生效，并同步公共技能"]
  K -- "暂不同步" --> M["保存结果保持生效，仅保留当前工作台修改"]
  J --> N["点击使用"]
  L --> N
  M --> N
  N --> O["进入当前工作台使用链路"]
...
2. 工作台中的当前工作台技能卡片点击后应进入可编辑配置弹窗，更多菜单应包含`编辑`、`生成应用`、`公开/取消公开`、`同步`、`导出`、`删除`，右下角主按钮为`使用`；已公开技能应展示`已公开`角标。
...
6. 技能库公共技能卡片点击后应进入可编辑配置弹窗，右下角主按钮为`发布`，更多菜单应只包含`编辑`、`取消公开`、`导出`；`发布`应进入发布确认弹窗，`取消公开`应从公共技能列表移除该技能。
7. 工作台中的已公开技能在保存基本信息或配置后，应追加“是否同步公共技能”的确认弹窗；无论选择`同步`还是`暂不同步`，当前工作台保存均已生效，仅确认同步时同步公共技能。
...
11. 除对话列表、任务列表、资源目录和结果目录中的删除确认外，工作台技能删除、公开/取消公开、同步、发布、未保存离开等二次确认均应展示为页面级模态弹窗。
