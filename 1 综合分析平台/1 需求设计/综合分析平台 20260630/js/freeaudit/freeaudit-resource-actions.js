(function () {
  ...
  NS.actionGroups.resourceActions = {
        ...
        toggleWorkbenchV2DirectoryDrawer(key) {
          const k = String(key || '').trim();
          if (!['material', 'result'].includes(k)) return;
          const open = this.workbenchV2DirectoryDrawerOpen || { material: true, result: true };
          this.workbenchV2DirectoryDrawerOpen = { ...open, [k]: !open[k] };
        },
        ...
        setWorkbenchEmbedMode(mode) {
          ...
          if (next === 'v2') {
            ...
            this.workbenchV2RightPanel = 'file';
            this.workbenchV2DirectoryDrawerOpen = { material: true, result: true };
            this.syncWorkbenchV2ResourceDrawerOpen('file');
            this.$nextTick(() => this.syncWorkbenchV2DetailHostReady());
          } else {
            this.workbenchV2DetailHostReady = false;
          }
        },
        syncWorkbenchV2DetailHostReady() {
          if (this.workbenchEmbedMode !== 'v2') {
            if (this.workbenchV2DetailHostReady) this.workbenchV2DetailHostReady = false;
            if (this.workbenchV2RightDrawerHostReady) this.workbenchV2RightDrawerHostReady = false;
            return false;
          }
          const detailReady = typeof document !== 'undefined'
            && !!document.getElementById('workbench-v2-detail-host');
          const drawerReady = typeof document !== 'undefined'
            && !!document.getElementById('workbench-v2-right-drawer-host');
          if (detailReady !== this.workbenchV2DetailHostReady) {
            this.workbenchV2DetailHostReady = detailReady;
          }
          if (drawerReady !== this.workbenchV2RightDrawerHostReady) {
            this.workbenchV2RightDrawerHostReady = drawerReady;
          }
          return detailReady || drawerReady;
        },
        ...
        onWorkbenchV2RailSelect(panel) {
          const kind = String(panel || '').trim();
          if (kind === 'toggle') {
            if (this.workbenchEmbedMode === 'v2') return;
            if (!this.workbenchV2RightDrawerCollapsed) {
        ...
        closeResourcePreview() {
          if (this.workbenchEmbedMode === 'v2' && this.workbenchV2DetailActiveTabKey) {
            ...
            this.resourcePreviewTab = 'schema';
            return;
          }
        ...
        previewResource(type, row) {
          ...
          this.resourcePreviewTab = type === 'database' ? 'schema' : 'topology';
          if (this.workbenchEmbedMode === 'v2') {
            this.sourcesLeftView = 'list';
            this.sourcesRightView = 'detail';
            this.studioCollapsed = false;
            this.registerWorkbenchV2DetailTabForResource(type, row);
          } else {
        ...
        workbenchResourceBasicInfoRows(row) {
          const r = row || {};
          ...
          const rowCountLabel = String(r.rowCountLabel || '').trim()
            || (Number.isFinite(rowCount)
              ...
              : '—');
          const rows = [
            { label: '资源类型', value: type === 'database' ? '数据库' : '数据图谱' },
            { label: '名称', value: type === 'database' && databaseLabel && tableLabel ? `${databaseLabel} / ${tableLabel}` : String(r.name || '').trim() || '—' },
          ];
          if (type === 'database') {
        ...
        openWorkbenchResourceBasicInfo(type, row) {
          if (!row || typeof this.openWorkbenchBasicInfoModal !== 'function') return;
          const payload = type === 'database'
            ? { ...this.buildDbTablePreviewPayload(row), type: 'database' }
            : { ...row, type };
          const name = String(payload.name || '资源').trim() || '资源';
          this.openWorkbenchBasicInfoModal(`${name} 详情`, this.workbenchResourceBasicInfoRows(payload));
        },
        onWorkbenchDbTableContextMenu(key, tbl) {
          ...
          if (key === 'detail') {
            this.openWorkbenchResourceBasicInfo('database', tbl);
            return;
          }
        ...
        onWorkbenchGraphContextMenu(key, graph) {
          if (key === 'detail') {
            this.openWorkbenchResourceBasicInfo('graph', graph);
            return;
          }
        ...
        onResourcePreviewContextMenu(key, row) {
          if (!row) return;
          if (key === 'detail') {
            this.openWorkbenchResourceBasicInfo(row.type, row);
            return;
          }
        ...
        openWbTaskResourcePreview(item) {
          if (this.workbenchEmbedMode === 'v2') {
            this.openWbTaskResourceInV2Preview(item);
            return;
          }
          ...
          this.wbTaskResourcePreviewActiveTab = this.wbTaskConfigResolveResourceKind(item) === 'database' ? 'fields' : 'preview';
          this.wbTaskResourcePreviewOcrLines = false;
          this.wbTaskResourcePreviewOpen = true;
        },
        wbResolveTaskResultMaterialVm(item) {
          if (!item || typeof item !== 'object') return null;
          const key = String(item.key != null ? item.key : '').trim();
          const idFromKey = key.startsWith('result:') ? key.slice('result:'.length).trim() : '';
          const targetId = idFromKey || String(item.id || '').trim();
          if (!targetId) return null;
          const mats = this.materials || [];
          for (let i = 0; i < mats.length; i += 1) {
            const m = mats[i];
            if (!m || m.type !== 'analysis') continue;
            const id = String(m.id || '').trim();
            const psId = m.projectSource && String(m.projectSource.id || '').trim();
            if (id === targetId || psId === targetId) return m;
          }
          return null;
        },
        openWbTaskResourceInV2Preview(item) {
          if (!item || typeof item !== 'object') return;
          const kind = this.wbTaskConfigResolveResourceKind(item);
          if (kind === 'file') {
            const vm = this.wbResolveTaskResourceMaterialVm(item);
            if (vm) {
              this.openMaterialDetail(vm);
              return;
            }
          }
          if (kind === 'result') {
            const vm = this.wbResolveTaskResultMaterialVm(item);
            if (vm) {
              this.openMaterialDetail(vm);
              return;
            }
          ...
          if (kind === 'database') {
            const tables = this.wbTaskResourceResolveDatabasePreviewTables(item);
            const key = String(item.key != null ? item.key : '').trim();
            const idFromKey = key.startsWith('database:') ? key.slice('database:'.length).trim() : '';
            const targetId = idFromKey || String(item.id || '').trim();
            const list = this.dbResourceList || [];
            const hit = list.find((row) => String((row && row.id) || '').trim() === targetId);
            const row = hit
              ? { ...hit, tables: (hit.tables && hit.tables.length ? hit.tables : tables) }
              : {
                id: targetId || key || 'database-table',
                name: this.wbTaskConfigResourceDisplayTitle(item),
                tableName: item.physicalTable || item.tableName || item.logicalTableName,
                tables,
              };
            this.previewResource('database', row);
            return;
          }
          if (kind === 'graph') {
            const key = String(item.key != null ? item.key : '').trim();
            const idFromKey = key.startsWith('graph:') ? key.slice('graph:'.length).trim() : '';
            const targetId = idFromKey || String(item.id || '').trim();
            const list = this.graphResourceList || [];
            const hit = list.find((row) => String((row && row.id) || '').trim() === targetId);
            const row = hit || {
              id: targetId || key || 'graph',
              name: this.wbTaskConfigResourceDisplayTitle(item),
              entityCount: item.entityCount,
              edgeCount: item.edgeCount,
            };
            this.previewResource('graph', row);
            return;
          }
          const resolved = this.wbResolveTaskResourceMaterialVm(item);
          this.wbTaskResourcePreviewItem = item;
          this.wbTaskResourcePreviewResolvedMaterial = resolved;
          this.wbTaskResourcePreviewActiveTab = kind === 'database' ? 'fields' : 'preview';
          this.wbTaskResourcePreviewOcrLines = false;
          this.wbTaskResourcePreviewOpen = true;
        },
        ...
        closeWbTaskResourcePreview() {
          ...
          this.wbTaskResourcePreviewActiveTab = 'fields';
          this.wbTaskResourcePreviewOcrLines = false;
        },
