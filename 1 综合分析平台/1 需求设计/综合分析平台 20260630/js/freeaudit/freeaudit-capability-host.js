      ...
      template: `<a-layout class="shell-main shell-main-flex-col workbench-v2-scope workbench-v2-capability-host"><a-layout-content class="content-reset-flex-col">
                ...
                <div v-if="isWorkbenchV2DetailPaneVisible" class="nlm-side-panel-body is-analysis-detail-open" style="display:flex;flex-direction:column;min-height:0;">
                  ...
                  >
                    <div class="tc-skill-modal-body tc-skill-modal-body--unified workbench-v2-skill-preview-embed__body">
                      <div v-if="wbProjectSkillConfigIsGeneral" class="wb-general-skill-config-page">
                        <div class="wb-general-skill-config-page__head">
                          <p class="wb-general-skill-config-page__summary">大模型调用该技能时，将核心基于skill.md（审计思路）进行技能的执行。</p>
                        </div>
                        <SkillConfigEditor
                          ...
                          nav-title="目录"
                          rule-title="skill.md"
                          analysis-placeholder="暂无配置说明"
                        />
                      </div>
                      <div v-else class="wb-audit-skill-config-modal__grid">
                            <section class="wb-audit-skill-config-card wb-audit-skill-config-card--objects">
                              <div class="skill-config-section-head">
                                <div class="skill-config-section-title-cluster">
                                  <h3 class="skill-config-col-title skill-config-col-title--with-count">
                                    <span>分析对象</span>
                                    <span class="nlm-stat-count-tag">{{ getWbProjectAuditSkillRules(wbDetailSelectedSkill).length }}</span>
                                  </h3>
                                </div>
                              </div>
                              <div class="skill-object-list-wrap skill-object-list-wrap--config wb-audit-skill-config-card__list">
                                <div
                                  v-for="(er, idx) in getWbProjectAuditSkillRules(wbDetailSelectedSkill)"
                                  :key="er.id"
                                  class="skill-object-row"
                                  :class="{ 'is-expanded': wbProjectSkillAuditExpandedRuleId === er.id }"
                                >
                                  <div class="skill-object-row-main" @click="toggleWbProjectSkillAuditRuleExpand(er.id)">
                                    <div class="skill-object-row-text">
                                      <div class="skill-object-title-line" :class="{ 'is-placeholder': !String(er.title || '').trim() }">{{ String(er.title || '').trim() || '（未填写材料类型）' }}</div>
                                      <div class="skill-object-body-preview" :class="{ 'is-placeholder': !String(er.body || '').trim() }">{{ String(er.body || '').trim() || '（未填写关键信息抽取规则）' }}</div>
                                    </div>
                                    <div class="skill-object-row-actions wb-audit-skill-config-row-actions">
                                      <a-button type="link" size="small" class="wb-audit-skill-config-row-link" @click.stop="toggleWbProjectSkillAuditRuleExpand(er.id)">{{ wbProjectSkillAuditExpandedRuleId === er.id ? '收起' : '展开' }}</a-button>
                                    </div>
                                  </div>
                                  <div v-show="wbProjectSkillAuditExpandedRuleId === er.id" class="skill-object-row-detail" @click.stop>
                                    <div class="skill-object-field-group">
                                      <div class="skill-object-field-label skill-object-field-label--inline">请输入材料类型名称（可输入多个）<span class="skill-required">*</span></div>
                                      <a-textarea
                                        v-model:value="er.title"
                                        :rows="5"
                                        :disabled="true"
                                        placeholder="例如：合同、付款台账、发票"
                                      />
                                    </div>
                                    <div class="skill-object-field-group">
                                      <div class="skill-object-field-label skill-object-field-label--inline">请描述针对这些材料的关键信息抽取规则<span class="skill-required">*</span></div>
                                      <a-textarea
                                        v-model:value="er.body"
                                        :rows="6"
                                        :disabled="true"
                                        placeholder="请描述需要抽取、识别或核对的关键字段与口径"
                                      />
                                    </div>
                                  </div>
                                </div>
                            ...
                            <section class="wb-audit-skill-config-card wb-audit-skill-config-card--rule">
                              <div class="skill-config-section-head">
                                <div class="skill-config-section-title-cluster">
                                  <h3 class="skill-config-col-title">分析规则</h3>
                                </div>
                              </div>
                              <a-textarea
                                v-model:value="wbDetailSelectedSkill.analysisRule"
                                class="wb-audit-skill-config-card__rule-textarea"
                                :rows="18"
                                :disabled="true"
                                placeholder="暂无分析规则"
                              />
                                  ...
                                  >
                                    <template v-if="selectedMaterialIsPackageDownloadTask">
                                      ...
                                      <section class="wb-task-detail-section">
                                        ...
                                        <div class="wb-task-detail-generate-req-box wb-task-detail-package-scope" role="region" aria-label="打包范围">
          ...
          >
            <a-form layout="vertical" class="skill-modal-form skill-wizard-panel">
              ...
              <a-form-item label="技能类型" required>
                <div class="wb-skill-kind-grid" role="radiogroup" aria-label="技能类型">
                  <button
                    v-for="item in wbProjectSkillKindOptions"
                    :key="item.id"
                    type="button"
                    role="radio"
                    class="wb-skill-kind-card"
                    :class="{ 'is-active': wbProjectSkillCreateBasicForm.skillKind === item.id }"
                    :aria-checked="wbProjectSkillCreateBasicForm.skillKind === item.id"
                    @click="wbProjectSkillCreateBasicForm.skillKind = item.id"
                  >
                    <span class="wb-skill-kind-card__title">{{ item.label }}</span>
                    <span class="wb-skill-kind-card__desc">{{ item.description }}</span>
                  </button>
                </div>
              </a-form-item>
              <div class="wb-skill-create-advanced">
                <button
                  ...
                  :aria-expanded="wbProjectSkillCreateAdvancedOpen"
                  @click="wbProjectSkillCreateAdvancedOpen = !wbProjectSkillCreateAdvancedOpen"
                >
                  <span>{{ wbProjectSkillCreateAdvancedOpen ? '收起详细信息' : '更多详细信息' }}</span>
                  <ds-icon
                    :name="wbProjectSkillCreateAdvancedOpen ? 'chevron-up' : 'chevron-down'"
                    class="wb-skill-create-advanced__icon"
                  />
                </button>
                <div v-show="wbProjectSkillCreateAdvancedOpen" class="wb-skill-create-advanced__body">
                  ...
                  <a-form-item :label="wbProjectSkillDimensionFieldLabel">
                    <a-select
                      v-model:value="wbProjectSkillCreateBasicForm.skillType"
                      :placeholder="'请选择' + wbProjectSkillDimensionFieldLabel"
                      style="width: 100%"
                    >
          ...
          <a-modal
                    ...
                    <a-form-item label="技能类型" required>
                      <div class="wb-skill-kind-grid" role="radiogroup" aria-label="技能类型">
                        <button
                          ...
                          role="radio"
                          class="wb-skill-kind-card"
                          :class="{ 'is-active': wbProjectSkillForm.skillKind === item.id }"
                          :aria-checked="wbProjectSkillForm.skillKind === item.id"
                          :disabled="wbProjectSkillBasicFieldsLocked"
                          @click="wbProjectSkillForm.skillKind = item.id"
                        >
                          <span class="wb-skill-kind-card__title">{{ item.label }}</span>
                          <span class="wb-skill-kind-card__desc">{{ item.description }}</span>
                        </button>
              ...
              <a-modal
                ...
                :width="wbProjectSkillConfigIsGeneral ? 1040 : 1460"
                :wrapClassName="'modal-skill-config modal-project-skill-config' + (wbProjectSkillConfigIsAudit ? ' modal-project-skill-config--audit' : '')"
                centered
                :maskClosable="false"
              ...
              >
                  ...
                  <div class="ds-unified-tab-pane-stack">
                    <div v-if="wbProjectSkillConfigIsGeneral" class="wb-general-skill-config-page">
                      ...
                      <SkillConfigEditor
                        ...
                        nav-title="目录"
                        rule-title="skill.md"
                        analysis-placeholder="请描述该通用技能需要完成的目标、适用输入、输出格式、引用约束和禁用项"
                        :show-save="!wbProjectSkillDetailReadOnly"
                        :save-disabled="!wbProjectSkillConfigPaneDirty"
                        @change="scheduleWbProjectSkillDetailSync"
                        @save="saveWbProjectSkillConfigPane"
                        @polish-rule="demoAiPolishWbProjectAnalysisRule"
                        @undo-rule="undoWbProjectAnalysisRule"
                        @polish-file-field="demoAiPolishWbSkillFileField"
                        @undo-file-field="undoWbSkillFileField"
                      />
                    </div>
                    <div v-else class="wb-audit-skill-config-page">
                      <div class="wb-audit-skill-config-page__head">
                        <p class="wb-audit-skill-config-page__title">大模型调用该技能时，将抽取分析对象的关键信息，再基于分析规则完成审计分析。</p>
                        <a-button
                      ...
                      <div class="wb-audit-skill-config-modal__grid">
                          <section class="wb-audit-skill-config-card wb-audit-skill-config-card--objects">
                            <div class="skill-config-section-head">
                              <div class="skill-config-section-title-cluster">
                                <h3 class="skill-config-col-title skill-config-col-title--with-count">
                              ...
                              <a-button v-if="!wbProjectSkillConfigTabLocked" type="default" @click="addWbProjectSkillAuditRule">添加</a-button>
                            </div>
                            <div class="skill-object-list-wrap skill-object-list-wrap--config wb-audit-skill-config-card__list">
                              <div
                                v-for="(er, idx) in getWbProjectAuditSkillRules(wbDetailSelectedSkill)"
                              ...
                              >
                                <div class="skill-object-row-main" @click="toggleWbProjectSkillAuditRuleExpand(er.id)">
                                  ...
                                  <div class="skill-object-row-actions wb-audit-skill-config-row-actions">
                                    <a-button type="link" size="small" class="wb-audit-skill-config-row-link" @click.stop="toggleWbProjectSkillAuditRuleExpand(er.id)">{{ wbProjectSkillAuditExpandedRuleId === er.id ? '收起' : '展开' }}</a-button>
                                    <a-button
                                      v-if="!wbProjectSkillConfigTabLocked"
                                      type="link"
                                      size="small"
                                      danger
                                      class="wb-audit-skill-config-row-link wb-audit-skill-config-row-link--danger"
                                      :disabled="getWbProjectAuditSkillRules(wbDetailSelectedSkill).length <= 1"
                                      @click.stop="removeWbProjectSkillAuditRule(idx)"
                                    >删除</a-button>
                                  </div>
                                </div>
                                <div v-show="wbProjectSkillAuditExpandedRuleId === er.id" class="skill-object-row-detail" @click.stop>
                                  ...
                                  <div class="skill-object-field-group">
                                    <div class="skill-object-field-label-row">
                                      ...
                                      <div v-if="!wbProjectSkillConfigTabLocked" class="skill-object-field-polish">
                                        <span v-show="freeAuditAiPolishKey === 'wb-project-er:' + er.id + ':body'" class="ds-input-ai-polish-status">润色中...</span>
                                        <button type="button" class="ds-icon-btn ds-input-ai-polish-undo-btn" aria-label="撤回" title="撤回至润色前" :disabled="freeAuditPolishUndoDisabled('wb-project-er:' + er.id + ':body')" @click.stop="undoWbProjectAuditRuleField(er, 'body')">
                                          <ds-icon name="arrow-rotate-left" aria-hidden="true" />
                                        </button>
                                        <button type="button" class="ds-icon-btn ds-input-ai-polish-btn" aria-label="智能润色" title="智能润色" :disabled="!!freeAuditAiPolishKey" @click.stop="demoAiPolishWbProjectAuditRuleField(er, 'body')">
                          ...
                          <section class="wb-audit-skill-config-card wb-audit-skill-config-card--rule">
                            <div class="skill-config-section-head">
                              ...
                              <div v-if="!wbProjectSkillConfigTabLocked" class="skill-object-field-polish">
                                ...
                                <button type="button" class="ds-icon-btn ds-input-ai-polish-undo-btn" aria-label="撤回" title="撤回至润色前" :disabled="freeAuditPolishUndoDisabled('wb-analysisRule')" @click.stop="undoWbProjectAnalysisRule">
                                  <ds-icon name="arrow-rotate-left" aria-hidden="true" />
                                </button>
                                <button type="button" class="ds-icon-btn ds-input-ai-polish-btn" aria-label="智能润色" title="智能润色" :disabled="!!freeAuditAiPolishKey" @click.stop="demoAiPolishWbProjectAnalysisRule">
                                  <iconpark-icon name="optimize" aria-hidden="true"></iconpark-icon>
                                </button>
                              </div>
                            </div>
                ...
                <template #footer>
                  <div v-if="wbProjectSkillConfigIsGeneral" class="ds-modal-footer-end">
                    <a-space>
