(function () {
  ...
  NS.actionGroups.taskActions = {
        ...
        submitWorkbenchTaskCreate() {
          ...
          const now = new Date().toISOString().slice(0, 19).replace('T', ' ');
          const projectId = String(this.workbenchProjectId || (getFreeAuditQuery().projectId) || 'PRJ-2026-001');
          if (taskType === 'batch') {
            ...
            this.workbenchCreatedTasks = [created, ...(this.workbenchCreatedTasks || [])];
            if (typeof demoWorkbenchTaskRows !== 'undefined' && Array.isArray(demoWorkbenchTaskRows) && !demoWorkbenchTaskRows.some((row) => row && row.id === created.id)) {
              demoWorkbenchTaskRows.unshift(created);
            }
            this.$emit && this.$emit('task-created', created);
            if (typeof window !== 'undefined' && typeof window.__DEMO_FREEAUDIT_TASK_CREATED_BRIDGE === 'function') {
              window.__DEMO_FREEAUDIT_TASK_CREATED_BRIDGE(created);
            }
