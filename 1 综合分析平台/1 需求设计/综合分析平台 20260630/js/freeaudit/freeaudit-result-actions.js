(function () {
  ...
  NS.actionGroups.resultActions = {
        ...
        handleTreeContextMenu(key, node, sectionKey, treeScope) {
          ...
          if (key === 'detail') {
            if (!raw || typeof this.openWorkbenchBasicInfoModal !== 'function') return;
            const name = String(
              (raw.projectSource && raw.projectSource.name)
              || raw.title
              || raw.name
              || '详情'
            ).trim() || '详情';
            ...
            if (sectionKey === 'material') {
              rows = this.materialBasicMetaRowsFromProjectSourceRow(raw.projectSource || raw);
            } else if (sectionKey === 'analysis') {
              rows = this.analysisResultBasicMetaRowsFromRecord(raw.projectSource || raw);
            }
            this.openWorkbenchBasicInfoModal(`${name} 详情`, rows);
            return;
          }
        ...
        handleWorkbenchMaterialAction(key, raw) {
          ...
          if (key === 'detail') {
            ...
            this.openWorkbenchBasicInfoModal(`${name} 详情`, this.materialBasicMetaRowsFromProjectSourceRow(raw.projectSource || raw));
            return;
          }
          if (key === 'ref') {
