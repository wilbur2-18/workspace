(function () {
  ...
  function openV2AppRunCreatedNotice() {
    const content = '后续可通过页面左侧任务列表或应用内的生成记录进行任务状态的跟踪';
  ...
  app.component('FreeAuditWorkbenchV2', {
      ...
      >
          ...
          <section class="workbench-v2-conversation-list" aria-label="历史对话和任务列表">
            ...
            >
              ...
              <div
                ...
                :class="{ 'is-active': activeMainView === 'task' && item.id === activeTaskId, 'is-bulk-mode': workbenchBulkScopeActive('task', 'task'), 'is-bulk-selected': workbenchBulkIsSelected(sidebarTaskBulkDescriptor(item)) }"
                role="button"
                tabindex="0"
        ...
        <main class="workbench-v2-main" data-tour-id="workbench-main">
          ...
          >
            ...
            <template v-if="v2AppStage === 'list'">
              ...
              <section class="workbench-v2-app-grid workbench-v2-skill-grid" aria-label="应用列表">
                <article
                  ...
                  role="button"
                  tabindex="0"
                  @click="openV2AppCard(app)"
                  @keydown.enter.prevent="openV2AppCard(app)"
                  @keydown.space.prevent="openV2AppCard(app)"
                >
                  <div class="tc-template-card__body">
                    ...
                    >
                      ...
                      <div class="tc-template-card__footer-right workbench-v2-app-card__actions">
                        <button type="button" class="workbench-v2-app-card__primary" @click.stop="openV2AppUse(app)">进入</button>
                      </div>
                    </div>
                  </div>
                </article>
            ...
            <template v-else-if="v2AppStage === 'config' || v2AppStage === 'use'">
              <header
                v-if="!isSimpleWorkbenchMode || v2AppStage === 'config'"
              ...
              >
                ...
                >
                  <button
                    ...
                    role="tab"
                    :aria-selected="v2AppStage === 'config'"
                    @click="openV2AppConfigPage(activeV2App)"
                  >
                    配置
                  </button>
                  <button
                    type="button"
                    :class="{ 'is-active': v2AppStage === 'use' }"
                    role="tab"
                    :aria-selected="v2AppStage === 'use'"
                    @click="openV2AppUse(activeV2App)"
                  >
                    预览
                  </button>
                </div>
                  ...
                  <section class="workbench-v2-app-use-run" aria-label="应用执行">
                    <div class="workbench-v2-app-use-run__inner">
                      <div class="workbench-v2-app-run-toolbar">
                        <p class="workbench-v2-app-run-tip">请上传文件，上传后点击"生成结果"</p>
            ...
            <section v-else class="workbench-v2-app-record-reader" aria-label="执行记录结果阅读页">
              <article class="workbench-v2-app-record-reader__document" aria-label="结果文件阅读区">
          ...
          <a-modal
            ...
            centered
          >
            <div class="workbench-v2-app-example-preview">
    ...
    computed: {
      ...
      isPreviewView() {
        return this.isExpertWorkbenchMode && (this.activeMainView === 'chat' || this.activeMainView === 'task');
      },
      isRightDrawerOpen() {
    ...
    methods: {
      ...
      selectWorkbenchMode(mode) {
        ...
        this.activeMainView = next === 'simple' ? 'app' : 'chat';
        if (next === 'simple') {
      ...
      v2AppInCurrentScope(app) {
        ...
        if (this.isExpertWorkbenchMode) return owner === '我';
        return owner === '我' && app.status === 'published';
      },
      ...
      createV2AppFromEditor() {
        ...
        if (typeof message !== 'undefined' && message.success) {
          message.success(isEdit ? '已保存应用' : '已创建应用，请继续配置');
        }
      },
      ...
      openV2AppExamplePreview(fileName, type) {
        ...
        this.v2AppExamplePreview = {
          ...
          fileName: name,
          lines: isOutput
            ? ['识别 3 条合同变更链路、2 条金额差异线索。', '建议优先复核审批附件、台账金额与付款记录的一致性。']
            : ['示例文件用于说明应用期望的上传材料格式。', '系统将解析文件内容，并用于后续结果生成。'],
        };
      },
