(function () {
  ...
  panels.v2ResourcePanelOpen = function v2ResourcePanelOpen(kind, options) {
    const hasActions = !options || options.hasActions !== false;
    const drawerKey = options && options.drawerKey;
    const drawerState = drawerKey ? `workbenchV2DirectoryDrawerOpen.${drawerKey}` : '';
    const sectionClass = drawerKey ? ' workbench-v2-directory-drawer' : '';
    const sectionBind = drawerKey ? ` :class="{ 'is-open': ${drawerState} }"` : '';
    const toggleClass = drawerKey ? ' workbench-v2-directory-drawer__toggle' : '';
    const toggleAttrs = drawerKey
      ? ` role="button" tabindex="0" :aria-expanded="${drawerState} ? 'true' : 'false'" @click="toggleWorkbenchV2DirectoryDrawer('${drawerKey}')" @keydown.enter.prevent="toggleWorkbenchV2DirectoryDrawer('${drawerKey}')" @keydown.space.prevent="toggleWorkbenchV2DirectoryDrawer('${drawerKey}')"`
      : '';
    const toggleIcon = drawerKey
      ? `<ds-icon name="chevron-right" class="workbench-v2-directory-drawer__chevron" :class="{ 'is-expanded': ${drawerState} }" aria-hidden="true" />`
      : '';
    if (hasActions) {
      return `
      <section class="nlm-right-split-section nlm-resource-panel workbench-v2-resource-panel${sectionClass}"${sectionBind}>
        <div class="nlm-panel-header">
          <div class="nlm-panel-title-row nlm-panel-title-row--result-with-refresh">
            <span class="nlm-stat-count-label-row${toggleClass}"${toggleAttrs}>
              ${toggleIcon}
  ...
  panels.v2ResourcePanelBodyOpen = function v2ResourcePanelBodyOpen(kind, options) {
    const drawerKey = options && options.drawerKey;
    const bodyShow = drawerKey ? ` v-show="workbenchV2DirectoryDrawerOpen.${drawerKey}"` : '';
    return `
        <div${bodyShow} :ref="(el) => setResourceDrawerBodyRef('${kind}', el)" class="nlm-resource-panel__body">`;
  };
  ...
  panels.rightResultSectionOpen = function rightResultSectionOpen(options) {
    const drawerKey = options && options.drawerKey;
    const drawerState = drawerKey ? `workbenchV2DirectoryDrawerOpen.${drawerKey}` : '';
    const sectionClass = drawerKey ? ' workbench-v2-directory-drawer' : '';
    const sectionBind = drawerKey ? ` :class="{ 'is-open': ${drawerState} }"` : '';
    ...
    const toggleIcon = drawerKey
      ? `<ds-icon name="chevron-right" class="workbench-v2-directory-drawer__chevron" :class="{ 'is-expanded': ${drawerState} }" aria-hidden="true" />`
      : '';
    const bodyShow = drawerKey ? ` v-show="${drawerState}"` : '';
    return `
      <section class="nlm-right-split-section nlm-right-split-section--result${sectionClass}" :style="rightSplitResultSectionStyle"${sectionBind}>
        <div class="nlm-panel-header">
          <div class="nlm-panel-title-row nlm-panel-title-row--result-with-refresh">
            <span class="nlm-stat-count-label-row${toggleClass}"${toggleAttrs}>
              ${toggleIcon}
        ...
        <div${bodyShow} class="nlm-material-toolbar">
        ...
        <div${bodyShow} class="nlm-fill-scroll nlm-right-split-scroll wb-material-file-drawer" style="flex:1;min-height:0;" @mouseleave="hoveredMaterialId = null" @click="onMaterialListAreaClick($event)">
    `;
  };
