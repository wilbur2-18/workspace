(function () {
  ...
  app.component('ChatMessage', {
    ...
    computed: {
      ...
      showActions() {
        return this.msg.role === 'bot' && !this.msg.suppressActions && !this.msg.resultDecision;
      },
    },
    methods: {
          ...
          <div :key="msg.id" class="nlm-msg-row">
            <div class="nlm-msg-wrap">
              ...
              <button
                v-if="msg.role === 'user'"
                type="button"
                class="nlm-msg-copy-btn nlm-msg-copy-btn--user"
                title="复制"
                aria-label="复制用户消息"
                @click.stop="host.copyMessage(msg)"
              >
                <ds-icon name="copy" aria-hidden="true" />
              </button>
              ...
              <div v-if="showActions" class="msg-actions">
                <button v-if="!msg.chatIntro" type="button" class="msg-action-btn" title="总结为技能" aria-label="总结为技能" @click="host.openSummarizeToTemplateModal(msg)"><ds-icon name="file-contract" aria-hidden="true" /></button>
                <button type="button" class="msg-action-btn" title="复制" aria-label="复制回答" @click="host.copyMessage(msg)"><ds-icon name="copy" aria-hidden="true" /></button>
                <button v-if="!msg.chatIntro" type="button" class="msg-action-btn" title="保存到结果" aria-label="保存到结果" @click="host.openSaveResultModal(msg)"><ds-icon name="save" aria-hidden="true" /></button>
              </div>
            </div>
          </div>
        </transition>
