(function () {
  ...
  app.component('ProjectCenterCard', {
      ...
      >
        ...
        <div class="ds-list-card-body">
          ...
          <div class="ds-card-foot" @click.stop>
            <div class="project-center-card-meta" :title="projectCardMetaTitle(project)">
              <span class="project-center-card-meta__item project-center-card-meta__creator" :aria-label="'创建人：' + projectCardCreatorLabel(project)">
                <ds-icon name="user" class="project-center-card-meta__icon" aria-hidden="true" />
                <span class="project-center-card-meta__text">{{ projectCardCreatorLabel(project) }}</span>
              </span>
              <span class="project-center-card-meta__item project-center-card-meta__updated" :aria-label="'最近更新：' + projectCardUpdatedLabel(project)">
                <ds-icon name="clock" class="project-center-card-meta__icon" aria-hidden="true" />
                <span class="project-center-card-meta__text tabular-nums">{{ projectCardUpdatedLabel(project) }}</span>
              </span>
            </div>
    ...
    `,
    methods: {
      projectCardCreatorLabel(project) {
        const creator = String((project && project.createdBy) || '—').trim() || '—';
        return creator;
      },
      projectCardUpdatedLabel(project) {
        const raw = project && (project.updatedAt || project.createdAt);
        return this.formatProjectCardTime(raw);
      },
      projectCardMetaTitle(project) {
        const creator = String((project && project.createdBy) || '—').trim() || '—';
        const createdAt = String((project && project.createdAt) || '').trim() || '—';
        const updatedAt = String((project && (project.updatedAt || project.createdAt)) || '').trim() || '—';
        return `创建人：${creator}；创建时间：${createdAt}；最近更新：${updatedAt}`;
      },
      formatProjectCardTime(raw) {
        ...
        if (year === currentYear) return `${month}-${day}${hour && minute ? ` ${hour}:${minute}` : ''}`;
  ...
  app.component('ProjectCenterListPanel', {
    ...
    template: `
      <div class="main-container">
          <header class="ds-page-hero ds-page-hero--l1 ds-page-hero--with-action" aria-label="工作台">
            <div class="ds-page-hero__main">
