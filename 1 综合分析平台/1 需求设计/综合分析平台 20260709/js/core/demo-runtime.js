    ...
    const DEFAULT_WORKBENCH_PROJECT_ID = 'PRJ-2026-001';
    const DEFAULT_WORKBENCH_HASH = 'freeaudit?projectId=' + encodeURIComponent(DEFAULT_WORKBENCH_PROJECT_ID);
    ...
    const getHashView = () => {
      ...
      if (!raw) return VIEW_IDS.FREE_AUDIT;
      const hash = raw.split('?')[0] || VIEW_IDS.FREE_AUDIT;
    ...
    window.__demoPolishSampleForKey = function (key) {
      if (key === 'summary:name' || /:(title|fn)$/.test(String(key || ''))) return window.__DEMO_AI_POLISH_SAMPLE_SHORT;
