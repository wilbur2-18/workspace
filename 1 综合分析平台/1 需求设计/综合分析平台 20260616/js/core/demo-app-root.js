    window.__DEMO_APP = createApp({
      ...
      data() {
        ...
        return {
          ...
          newProjectForm: { name: '', description: '' },
      ...
      methods: {
        ensureDefaultRoute() {
          ...
          window.location.hash = DEFAULT_WORKBENCH_HASH;
        },
        navigate(viewId) {
          ...
          if (viewId === VIEW_IDS.FREE_AUDIT && (base === 'freeaudit' || base === 'freeaudit-v2')) return;
          window.location.hash = VIEW_TO_HASH[viewId] || 'project';
        },
        openNewProjectModal() {
          this.newProjectForm = { name: '', description: '' };
