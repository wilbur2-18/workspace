(function () {
  ...
  function buildTemplateSkillForWorkbench(row, tab) {
    ...
    next.sourceLabel = tab === TEMPLATE_SKILL_LIBRARY.MARKET ? '技能市场' : '共享技能';
    next.sourceSkillName = row.name || next.name || '';
    next.sourceVersionLabel = tab === TEMPLATE_SKILL_LIBRARY.MARKET ? '技能市场' : '共享技能';
    next.createdAt = now;
    next.updatedAt = now;
    ...
    app.component('TemplateCenterView', {
      template: `
        <a-layout class="shell-main">
          <div class="app-main template-center-shell">
            ...
            <a-modal
              v-model:open="skillMarketIntakeModalOpen"
              title="市场技能入库"
              width="640"
              wrapClassName="modal-w-640"
              centered
              :maskClosable="false"
              @cancel="closeMarketIntakeModal"
            >
              <a-form layout="vertical" v-if="skillMarketIntakeSkill">
                <div class="template-market-source-box">
                  <div class="template-market-source-box__title">市场来源信息</div>
                  <div class="template-market-source-box__grid">
                    <span>来源</span><strong>{{ skillMarketIntakeSkill.sourceLabel || '上级技能市场' }}</strong>
                    <span>市场分类</span><strong>{{ skillMarketIntakeSkill.marketAuditScene || '未提供' }} / {{ skillMarketIntakeSkill.marketSkillType || '未提供' }}</strong>
                  </div>
                </div>
                <a-form-item label="共享技能名称" required>
                  <a-input v-model:value="skillMarketIntakeForm.name" allow-clear :maxlength="60" />
                </a-form-item>
                <a-form-item label="共享技能简介">
                  <a-textarea v-model:value="skillMarketIntakeForm.description" :rows="3" :maxlength="300" />
                </a-form-item>
                <div class="template-dimension-form-grid">
                  <a-form-item
                    v-for="category in skillEnabledCategoryRows"
                    :key="'market-' + category.id"
                    :label="category.label"
                    required
                  >
                    <a-select
                      v-model:value="skillMarketIntakeForm.dimensionValues[category.id]"
                      :placeholder="'请选择' + category.label"
                    >
                      <a-select-option v-for="item in getSkillDimensionOptions(category.id)" :key="item.id" :value="item.id">{{ item.label }}</a-select-option>
                    </a-select>
              ...
              <template #footer>
                <a-button @click="closeMarketIntakeModal">取消</a-button>
                <a-button type="primary" @click="submitMarketSkillIntake">入库</a-button>
              </template>
            </a-modal>
            ...
            >
              <div v-if="skillMarketPublishSkill" class="template-skill-publish-confirm">
                ...
                <a-descriptions bordered :column="1" size="small" class="template-skill-publish-confirm__table">
                  <a-descriptions-item label="技能名称">{{ skillMarketPublishSkill.name || '—' }}</a-descriptions-item>
                  <a-descriptions-item label="技能描述">{{ String(skillMarketPublishSkill.description || '').trim() || '暂无' }}</a-descriptions-item>
                  <a-descriptions-item label="标签">{{ skillMarketPublishTagsText }}</a-descriptions-item>
                </a-descriptions>
                <a-alert
                  type="info"
                  show-icon
                  class="template-skill-publish-confirm__notice"
                  message="提交后需等待远程技能市场管理员审核，审核结果将在申请列表中查看。"
                />
              </div>
              <template #footer>
      ...
      data() {
        return {
          ...
          skillMarketIntakeModalOpen: false,
          skillMarketIntakeSkill: null,
          skillMarketIntakeForm: { name: '', description: '', dimensionValues: {} },
      ...
      computed: {
        ...
        skillBasicModalTitleText() {
          if (this.skillBasicFormLocked) return '查看';
          return '编辑';
        },
        ...
        skillEmptyDescription() {
          if (!this.currentSkillList.length) {
            return this.skillLibraryTab === TEMPLATE_SKILL_LIBRARY.MARKET ? '暂无技能市场内容。' : '暂无共享技能。';
          }
      ...
      methods: {
        ...
        unshareSharedSkillCard(s) {
          ...
          if (this.isPublicSkillMineShared(s)) {
            window.dsConfirm.action({
              title: '取消共享？',
              content: '取消后，该技能将从共享技能库移除，你仍可在我创建的条目中继续维护。',
              okText: '取消共享',
              onOk: () => {
        ...
        submitSkillCategorySettings() {
          ...
          if (window.DemoSkillData) window.DemoSkillData.skillDimensions = skillDeepClone(this.skillDimensionSettings);
        ...
        openMarketSkillIntakeModal(skill) {
          if (!skill) return;
          if (this.isMarketSkillIntaked(skill)) {
            message.info('该市场技能已入库');
            return;
          }
          this.skillMarketIntakeSkill = skill;
          this.skillMarketIntakeForm = {
            name: String(skill.name || '').trim(),
            description: String(skill.description || '').trim(),
            dimensionValues: this.buildSkillDimensionForm(skill).dimensionValues,
          };
          this.skillMarketIntakeModalOpen = true;
        },
        closeMarketIntakeModal() {
          this.skillMarketIntakeModalOpen = false;
          this.skillMarketIntakeSkill = null;
          this.skillMarketIntakeForm = { name: '', description: '', dimensionValues: {} };
        },
        openSkillMarketPublishModal(skill) {
          if (!skill) return;
          this.skillMarketPublishSkill = skill;
        ...
        submitMarketSkillIntake() {
          const source = this.skillMarketIntakeSkill;
          if (!source) return;
          const name = String(this.skillMarketIntakeForm.name || '').trim();
          const values = (this.skillMarketIntakeForm && this.skillMarketIntakeForm.dimensionValues) || {};
          if (!name) {
            message.warning('请填写共享技能名称');
            return;
          }
          const missing = (this.skillEnabledCategoryRows || []).find((category) => !String(values[category.id] || '').trim());
          if (missing) {
          ...
          const now = templateNow();
          const copy = skillDeepClone(source);
          copy.id = newSkillId('sk-shared');
          copy.library = SKILL_LIBRARY.PUBLIC;
          copy.name = name;
          copy.description = String(this.skillMarketIntakeForm.description || '').trim();
          this.syncSkillDimensionValues(copy, values);
          copy.sourceKind = 'shared';
          copy.sourceLabel = '共享技能';
          copy.sourceSkillId = templateSkillSourceId(source) || source.id;
          copy.sourceLibrary = 'market';
          copy.sourceSkillName = source.name || name;
          copy.sourceVersionLabel = '市场入库';
          copy.libraryStatus = 'enabled';
          copy.sharedBy = '平台管理员';
          copy.createdAt = now;
          copy.updatedAt = now;
          ...
          this.closeMarketIntakeModal();
          message.success('已入库为共享技能');
        },
        ...
        onTemplateCardMenu(info, s) {
          ...
          if (key === 'publish') {
            this.openSkillMarketPublishModal(s);
            return;
          }
          if (key === 'edit') {
            const target = this.resolveCardSkillTarget(s, { readOnly: false });
          ...
          if (key === 'delete') {
            const priv = s && s.library === SKILL_LIBRARY.PRIVATE
              ? s
              : (this.isPublicSkillMineShared(s) ? this.resolvePrivateSkillForPublicMineShared(s) : null);
            if (!priv) return;
            window.dsConfirm.delete({
              subject: '该技能',
              onOk: () => {
                this.deleteSkill(priv);
              },
            });
          }
        },
          ...
          if (q.templateId) {
            ...
            this.$nextTick(() => {
              if (this.selectedSkill) {
                const s = this.selectedSkill;
                const marketSkill = isTemplateMarketSkill(s);
                const configReadOnly = marketSkill || q.mode === 'view';
                const basicReadOnly = marketSkill || q.mode === 'view';
        ...
        closeSkillCreateBasicModal() {
          if (this._skillCreateAwaitingBasic && this.skillConfigTemplateId) {
            if (typeof window !== 'undefined' && window.dsConfirm && window.dsConfirm.action) {
              window.dsConfirm.action({
                title: '放弃创建技能？',
        ...
        saveSkillBasicPane() {
          ...
          if (!allowedScene.has(auditScene)) {
