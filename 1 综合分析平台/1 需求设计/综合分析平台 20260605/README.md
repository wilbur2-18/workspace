...
- 常用路由：`#project`、`#template`、`#freeaudit?projectId=PRJ-2026-001`。`#freeaudit-v2?...` 为旧链接兼容别名，同样进入当前审计工作台。
...
- 路由：`demo.html#freeaudit?projectId=PRJ-2026-001`；`#freeaudit-v2?...` 仅作为兼容别名保留。
- 入口组件：`js/freeaudit/demo-cmp-freeaudit-v2.js` 注册 `FreeAuditWorkbenchV2`，由项目入口路由加载。
- 样式入口：`assets/css/freeaudit/workbench-v2.css`，只承载当前工作台外壳层样式；内部资源、任务、结果、技能、对话能力由 `js/freeaudit/freeaudit-capability-host.js` 和 `js/freeaudit/*` 提供。

### 目标

- 把审计工作台从“资源 / 任务 / 结果 / 技能分散管理”收敛为面向对话推进的工作界面，让用户围绕当前审计问题连续完成引用资料、发起任务、查看结果、沉淀技能。
- 保留审计助手已有的演示闭环和数据能力，不重写底层业务能力。
- 降低审计人员在多面板、多入口之间切换的成本：高频动作应靠近当前工作上下文，历史对话、任务进展和工作台级能力应能快速回到同一条审计脉络。
- 支持渐进演示：当前工作台作为外壳和流程组织层迭代，内部资源树、任务详情、结果预览、技能配置等复杂能力继续复用共享能力实现。

### 原则

- 外壳与能力分层：`demo-cmp-freeaudit-v2.js` 只负责当前工作台外壳和入口组织；业务状态、资源 / 任务 / 结果 / 技能动作优先留在 `js/freeaudit/*`，由 `freeaudit-capability-host.js` 作为内部宿主承载。
- 原型行为优先表达产品意图：README 只记录稳定目标、交互边界和维护约束；不要把临时视觉批注、像素值、hover 细节或演示快捷行为写成产品规则。
- 复用优先：生成技能、引用技能、任务创建、对话输入等已有能力，除非产品目标发生变化，否则通过桥接复用旧版方法，不另起一套重复实现。
- 入口要服务工作流：新增按钮或入口前先判断它是否帮助用户推进当前审计任务；不要为了视觉对称或占位而增加独立入口。
- 侧栏是导航与上下文索引，不承载完整业务面板。复杂配置、详情编辑和结果查看仍应由既有弹窗、抽屉或主体区域承载。
- `workbench-v2.css` 只维护当前工作台外壳层。内部能力面板样式不要迁入该文件，除非该样式确实属于外壳适配边界。
...
| `js/freeaudit/freeaudit-capability-host.js` | 审计工作台内部能力宿主：对话、输入框、弹窗、资源 / 结果 / 任务 / 技能操作承载。 |
| `js/freeaudit/demo-cmp-freeaudit-v2.js` | 审计工作台外壳：左侧栏、对话 / 任务索引、顶部生成技能入口、右侧工具栏和能力宿主嵌入。 |
...
Chrome headless 在当前机器上可能在输出 DOM 后触发后台 updater 进程，不一定自然退出。进行 `demo.html#project`、`#template`、`#freeaudit?projectId=PRJ-2026-001` smoke 时，使用临时 `--user-data-dir` 抓取关键 DOM 后清理对应临时 profile 进程即可；`#freeaudit-v2?...` 仅需作为兼容别名抽检。
