(function () {
  ...
  function notifyWorkbenchV2DetailTabsChanged(host) {
    const bridge = typeof window !== 'undefined' ? window.__DEMO_FREEAUDIT_V2_DETAIL_TABS_BRIDGE : null;
    if (typeof bridge === 'function') bridge(host);
  }
  ...
  NS.actionGroups.chatActions = {
        ...
        hasDailyGuideResource() {
          const hasUpload = (this.chatUploadAttachments || []).some((item) => item && String(item.status || '') !== 'failed');
          const hasRef = (this.chatInputRefItems || []).length > 0;
          const hasMaterial = (this.workbenchRawMaterialsForTree || []).length > 0;
          return !!(hasUpload || hasRef || hasMaterial);
        },
        isDailyGuideReminderSuppressed() {
          return !!this.chatDailyGuideReminderSuppressed;
        },
        fillDailyGuidePendingPrompt() {
          const prompt = String(this.chatDailyGuidePendingPrompt || '').trim();
          if (prompt && !String(this.chatInput || '').trim()) {
        ...
        startDailyGuideUploadPrompt(prompt, title) {
          this.chatDailyGuidePendingPrompt = String(prompt || '').trim();
          this.chatDailyGuidePendingTitle = String(title || '').trim();
          if (this.isDailyGuideReminderSuppressed()) {
            this.fillDailyGuidePendingPrompt();
            this.clearDailyGuidePrompt();
            return;
          }
          this.chatDailyGuideStage = 'upload';
          this.$nextTick(() => {
            const el = this.$refs.chatInputEl;
            if (el && typeof el.focus === 'function') el.focus();
          });
        },
        completeDailyGuideUploadPrompt() {
          ...
          this.chatDailyGuideStage = 'send';
          this.$nextTick(() => {
            const el = this.$refs.chatInputEl;
            if (el && typeof el.focus === 'function') el.focus();
          });
        },
        clearDailyGuidePrompt() {
          this.chatDailyGuideStage = '';
          this.chatDailyGuidePendingPrompt = '';
          this.chatDailyGuidePendingTitle = '';
        },
        disableDailyGuideReminder() {
          this.chatDailyGuideReminderSuppressed = true;
          this.fillDailyGuidePendingPrompt();
          this.clearDailyGuidePrompt();
        },
        ...
        patchChatUploadAttachment(uid, patch) {
          ...
          this.chatUploadAttachments = (this.chatUploadAttachments || []).map((item) => {
            ...
            if (String(item.status || '') !== 'ready' && String(next.status || '') === 'ready') readyAttachment = next;
            return next;
          });
          if (readyAttachment) this.completeDailyGuideUploadPrompt();
        },
        ...
        retryChatUploadAttachment(uid) {
          ...
          this.chatUploadAttachments = (this.chatUploadAttachments || []).map((item) => {
            if (!item || String(item.uid) !== key) return item;
            found = true;
            return { ...item, status: 'uploading', progress: 12, failureReason: '' };
          });
          if (found) this.startChatUploadAttachmentSimulation(key);
          this.focusChatInput();
        },
        ...
        viewExtractionResult(id) {
          ...
          if (this.workbenchEmbedMode === 'v2') {
            ...
            this.sourcesRightView = 'detail';
            this.studioCollapsed = false;
            this.registerWorkbenchV2DetailTabForExtraction(id);
          } else {
        ...
        openMaterialDetail(m) {
          ...
          if (m.type === 'analysis') {
            ...
            this.wbAnalysisResultPreviewActiveTab = isWorkbenchTaskSidebarRow ? 'task-config' : 'body';
          } else {
            this.wbMaterialPreviewActiveTab = 'preview';
          ...
          if (this.workbenchEmbedMode === 'v2') {
            this.registerWorkbenchV2DetailTabForMaterial(m);
          }
        },
        closeWorkbenchMaterialDetail() {
          ...
          this.workbenchAnalysisEmbedDraft = '';
          this._workbenchAnalysisEmbedSnap = '';
          if (this.workbenchEmbedMode === 'v2' && this.workbenchV2DetailActiveTabKey) {
            this.closeWorkbenchV2DetailTab(this.workbenchV2DetailActiveTabKey);
            return;
          }
          if (this.workbenchEmbedMode === 'v2') {
            this.sourcesRightView = 'list';
          } else if (this.sourcesRightView === 'detail' && this.selectedMaterial && this.selectedMaterial.type === 'analysis') {
            this.sourcesRightView = 'list';
          } else {
            this.sourcesLeftView = 'list';
          }
        ...
        onWorkbenchAnalysisResultPreviewTabUpdate(key) {
          const next = String(key || (this.selectedMaterialIsWorkbenchCreatedTask ? 'task-config' : 'body'));
        ...
        handleChatAnalysisInlineCitationClick(e) {
          ...
          if (this.workbenchEmbedMode === 'v2') {
            this.registerWorkbenchV2DetailTabForMaterial(target);
          }
        },
        ...
        ensureDefaultDemoConversation() {
          ...
          const hasDraft = String(this.chatInput || '').trim()
            || (this.chatInputRefItems || []).length
            || (this.chatUploadAttachments || []).length;
          if (hasDraft) return;
        ...
        applyWorkbenchNextSuggestion(item) {
          const data = item || {};
          const prompt = String(data.prompt || data.label || data || '').trim();
          if (!prompt) return;
          this.chatInput = prompt;
          this.$nextTick(() => {
            this.adjustInputHeight();
        ...
        onResizeMove(e) {
          ...
          if (this.resizing.side === 'rightSplit') {
            ...
            const usableHeight = Math.max(1, panelHeight - 1);
        ...
        openWorkbenchBasicInfoModal(title, rows) {
          const safeRows = Array.isArray(rows) ? rows.filter(Boolean) : [];
          if (!safeRows.length) {
            message.info('暂无基本信息');
            return;
          }
          this.wbDetailBasicInfoModalTitle = String(title || '详情').trim() || '详情';
          this.wbDetailBasicInfoModalRows = safeRows;
          this.wbDetailBasicInfoModalOpen = true;
        },
        openActiveWorkbenchBasicInfoModal() {
          if (this.selectedResourcePreview) {
            const name = String(this.selectedResourcePreview.name || '资源').trim() || '资源';
            this.openWorkbenchBasicInfoModal(`${name} 详情`, this.resourcePreviewBasicMetaRows);
            return;
          }
          if (this.selectedMaterial && this.selectedMaterial.type === 'raw') {
            const name = String(
              (this.workbenchSelectedProjectMaterialRow && this.workbenchSelectedProjectMaterialRow.name)
              || this.selectedMaterial.title
              || '资料'
            ).trim() || '资料';
            this.openWorkbenchBasicInfoModal(`${name} 详情`, this.workbenchMaterialPreviewBasicMetaRows);
            return;
          }
          if (this.selectedMaterial && this.selectedMaterial.type === 'analysis') {
            const name = String(this.workbenchEmbedRightAnalysisHeaderTitle || this.selectedMaterial.title || '结果').trim() || '结果';
            this.openWorkbenchBasicInfoModal(`${name} 详情`, this.workbenchEmbedBasicMetaRows);
            return;
          }
          message.info('暂无基本信息');
        },
        closeWorkbenchBasicInfoModal() {
          this.wbDetailBasicInfoModalOpen = false;
          this.wbDetailBasicInfoModalRows = [];
        },
        _workbenchV2DetailTabTitleForMaterial(m) {
          if (!m) return '详情';
          if (m.type === 'analysis') {
            const row = (this.workbenchSelectedAnalysisResultRow || {});
            return String(row.name || m.title || '结果').trim() || '结果';
          }
          const row = this.workbenchSelectedProjectMaterialRow;
          if (row && row.name) return String(row.name).trim();
          return String(m.title || '资料').trim() || '资料';
        },
        _workbenchV2DetailTabTitleForResource(row) {
          return String((row && row.name) || '资源预览').trim() || '资源预览';
        },
        _workbenchV2DetailTabTitleForExtraction(item) {
          const name = String((item && item.dataSourceName) || '抽取').trim() || '抽取';
          return name + ' 抽取结果';
        },
        _saveWorkbenchV2DetailTabInnerState() {
          const key = this.workbenchV2DetailActiveTabKey;
          if (!key) return;
          this.workbenchV2DetailTabInnerState = {
            ...this.workbenchV2DetailTabInnerState,
            [key]: {
              materialPreviewTab: this.wbMaterialPreviewActiveTab,
              analysisPreviewTab: this.wbAnalysisResultPreviewActiveTab,
              resourcePreviewTab: this.resourcePreviewTab,
            },
          };
        },
        _restoreWorkbenchV2DetailTabInnerState(key) {
          const saved = (this.workbenchV2DetailTabInnerState || {})[key];
          if (!saved) return;
          if (saved.materialPreviewTab && saved.materialPreviewTab !== 'basic') this.wbMaterialPreviewActiveTab = saved.materialPreviewTab;
          if (saved.analysisPreviewTab && saved.analysisPreviewTab !== 'basic') this.wbAnalysisResultPreviewActiveTab = saved.analysisPreviewTab;
          if (saved.resourcePreviewTab && saved.resourcePreviewTab !== 'basic') this.resourcePreviewTab = saved.resourcePreviewTab;
        },
        _applyWorkbenchV2DetailTabSelection(tab) {
          if (!tab) return;
          if (tab.kind !== 'skill') {
          ...
          if (tab.kind === 'material') {
            this.selectedMaterialId = tab.materialId;
            const material = (this.materials || []).find((x) => x && x.id === tab.materialId);
            if (material && material.type === 'analysis') {
              const isTask = !!(
                (Array.isArray(this.workbenchCreatedTasks) && this.workbenchCreatedTasks.some((t) => t && t.id === material.id))
                || (Array.isArray(this.workbenchTaskDemoRows) && this.workbenchTaskDemoRows.some((t) => t && t.id === material.id))
              );
              this.wbAnalysisResultPreviewActiveTab = isTask ? 'task-config' : 'body';
            } else {
              this.wbMaterialPreviewActiveTab = 'preview';
            }
            this._restoreWorkbenchV2DetailTabInnerState(tab.key);
            if (tab.materialId && (this.materials || []).find((x) => x && x.id === tab.materialId && x.type === 'analysis')) {
              this.$nextTick(() => this.resetWorkbenchAnalysisEmbedDraftFromPreview());
            }
            return;
          }
          if (tab.kind === 'resource') {
            ...
            this.selectedResourcePreview = { ...row, type: tab.resourceType };
            this.resourcePreviewTab = tab.resourceType === 'database' ? 'schema' : 'topology';
            this._restoreWorkbenchV2DetailTabInnerState(tab.key);
            return;
          }
          if (tab.kind === 'extraction') {
            this.selectedExtractionId = tab.extractionId;
            return;
          }
          if (tab.kind === 'skill') {
            if (typeof this._prepareWbProjectSkillV2Preview === 'function') {
              this._prepareWbProjectSkillV2Preview({ id: tab.skillId });
            }
            this._restoreWorkbenchV2DetailTabInnerState(tab.key);
          }
        },
        activateWorkbenchV2DetailTab(key) {
          const nextKey = String(key || '').trim();
          if (!nextKey || nextKey === this.workbenchV2DetailActiveTabKey) return;
          const tab = (this.workbenchV2DetailTabs || []).find((t) => t && t.key === nextKey);
          if (!tab) return;
          this._saveWorkbenchV2DetailTabInnerState();
          this.workbenchV2DetailActiveTabKey = nextKey;
          this._applyWorkbenchV2DetailTabSelection(tab);
          this.sourcesRightView = 'detail';
          this.sourcesLeftView = 'list';
          this.studioCollapsed = false;
          notifyWorkbenchV2DetailTabsChanged(this);
        },
        openWorkbenchV2DetailTab(def) {
          if (!def || !def.key || this.workbenchEmbedMode !== 'v2') return;
          const key = String(def.key);
          const tabs = Array.isArray(this.workbenchV2DetailTabs) ? [...this.workbenchV2DetailTabs] : [];
          const idx = tabs.findIndex((t) => t && t.key === key);
          if (idx >= 0) {
            tabs[idx] = { ...tabs[idx], ...def, key };
          } else {
            tabs.push({ ...def, key });
          }
          this.workbenchV2DetailTabs = tabs;
          if (this.workbenchV2DetailActiveTabKey && this.workbenchV2DetailActiveTabKey !== key) {
            this._saveWorkbenchV2DetailTabInnerState();
          }
          this.workbenchV2DetailActiveTabKey = key;
          if (!this.workbenchV2DetailTabInnerState[key]) {
            this.workbenchV2DetailTabInnerState = {
              ...this.workbenchV2DetailTabInnerState,
              [key]: {
                materialPreviewTab: this.wbMaterialPreviewActiveTab,
          ...
          const activeTab = tabs.find((t) => t && t.key === key);
          if (activeTab) {
            this._applyWorkbenchV2DetailTabSelection(activeTab);
          }
          this.sourcesRightView = 'detail';
          this.sourcesLeftView = 'list';
          this.studioCollapsed = false;
        ...
        closeWorkbenchV2DetailTab(key) {
          const tabKey = String(key || '').trim();
          if (!tabKey) return;
          const tabs = Array.isArray(this.workbenchV2DetailTabs) ? this.workbenchV2DetailTabs : [];
          const idx = tabs.findIndex((t) => t && t.key === tabKey);
          if (idx < 0) return;
          const wasActive = this.workbenchV2DetailActiveTabKey === tabKey;
          const nextTabs = tabs.filter((t) => t && t.key !== tabKey);
          const innerState = { ...(this.workbenchV2DetailTabInnerState || {}) };
          delete innerState[tabKey];
          this.workbenchV2DetailTabInnerState = innerState;
          this.workbenchV2DetailTabs = nextTabs;
          if (!wasActive) {
            notifyWorkbenchV2DetailTabsChanged(this);
            return;
          }
          if (nextTabs.length) {
            ...
            this.workbenchV2DetailActiveTabKey = nextTab.key;
            this._applyWorkbenchV2DetailTabSelection(nextTab);
            this.sourcesRightView = 'detail';
            notifyWorkbenchV2DetailTabsChanged(this);
            return;
          }
          this.workbenchV2DetailActiveTabKey = null;
          this.selectedMaterialId = null;
          this.selectedResourcePreview = null;
          this.selectedExtractionId = null;
          this.selectedTreeNode = null;
        ...
        registerWorkbenchV2DetailTabForMaterial(m) {
          if (!m || !m.id || this.workbenchEmbedMode !== 'v2') return;
          this.openWorkbenchV2DetailTab({
            key: 'material:' + m.id,
            kind: 'material',
            title: this._workbenchV2DetailTabTitleForMaterial(m),
            materialId: m.id,
          });
        },
        registerWorkbenchV2DetailTabForResource(type, row) {
          if (!row || this.workbenchEmbedMode !== 'v2') return;
          const resourceKey = String(row.id || row.name || row.databaseId || 'resource');
          const scopeMap = { database: '库表', graph: '数据图谱', knowledge: '知识库' };
          this.openWorkbenchV2DetailTab({
            key: 'resource:' + String(type || 'resource') + ':' + resourceKey,
            kind: 'resource',
            title: this._workbenchV2DetailTabTitleForResource(row),
            scopeLabel: scopeMap[String(type || '')] || '资源',
            resourceType: String(type || 'resource'),
            resourceRow: { ...row },
          });
        },
        registerWorkbenchV2DetailTabForExtraction(id) {
          const extractionId = String(id || '').trim();
          if (!extractionId || this.workbenchEmbedMode !== 'v2') return;
          const item = (this.extractionResults || []).find((r) => r && r.id === extractionId);
          this.openWorkbenchV2DetailTab({
            key: 'extraction:' + extractionId,
            kind: 'extraction',
            title: this._workbenchV2DetailTabTitleForExtraction(item || { dataSourceName: '抽取' }),
            extractionId,
          });
        },
        registerWorkbenchV2DetailTabForSkill(skillId, title) {
          const sid = String(skillId || '').trim();
          if (!sid || this.workbenchEmbedMode !== 'v2') return;
          const pid = this.workbenchProjectId;
          const skill = pid && typeof demoProjectAnalysisTemplatesById !== 'undefined'
            ? (demoProjectAnalysisTemplatesById[pid] || []).find((x) => x && x.id === sid)
            : null;
          this.openWorkbenchV2DetailTab({
            key: 'skill:' + sid,
            kind: 'skill',
            title: String(title || (skill && skill.name) || '技能').trim() || '技能',
            skillId: sid,
          });
        }
  };
