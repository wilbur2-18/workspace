...
export function WorkbenchSidebar(props: Props) {
  return <aside className="wb-sidebar" aria-label="工作台对话与任务">
      <div className="wb-brand">
        <button type="button" className="wb-brand-mark" title={props.collapsed ? '展开左栏' : '浙江审计综合分析'} aria-label={props.collapsed ? '展开左栏' : '浙江审计综合分析'} onClick={props.collapsed ? props.onExpand : undefined}>
      ...
      <section className="wb-sidebar-list" aria-label="历史对话和任务列表">
        ...
        <div className="wb-list">
          {conversations.map(item => <button key={item.id} type="button" className={item.id === props.activeConversation ? 'is-active' : ''} onClick={() => props.onConversation(item.id)}>
              <Icon name="chat" />
        ...
        <div className="wb-list wb-task-list">
          {tasks.map(item => <button key={item.id} type="button" className={item.id === props.activeTask ? 'is-active' : ''} onClick={() => props.onTask(item.id)} onContextMenu={event => {
          event.preventDefault();
          props.onMenu(item.title);
        }}>
              <Icon name={item.icon} />
              <span>{item.title}</span>
