...
export function WorkbenchShell() {
  ...
  const shellClass = [
    ...
    docOpen ? 'is-doc-open' : '',
    docFullscreen ? 'is-doc-fullscreen' : '',
  ].filter(Boolean).join(' ');
      ...
      <main className="wb-main">
        <header className="wb-header">
          ...
          <div className="wb-header-actions">
            {activeView === 'chat' && (
              <button type="button" onClick={() => setModal('生成技能配置')}>
                <Icon name="brain" />
                <span>生成技能</span>
              </button>
            )}
            ...
            {activeView === 'task' && (
              <button type="button" onClick={() => setTaskDetailOpen((value) => !value)}>
                <Icon name="info" />
                <span>{taskDetailOpen ? '隐藏任务详情' : '显示任务详情'}</span>
              </button>
            )}
            {(activeView === 'chat' || activeView === 'task') && (
              <button
                type="button"
                className={`wb-icon-btn ${docOpen ? 'is-active' : ''}`}
                aria-label={docOpen ? '收起资料工作区' : '展开资料工作区'}
      ...
      {docOpen && (
        <>
          ...
          <WorkbenchDocWorkspace
            ...
            onFullscreen={() => setDocFullscreen((value) => !value)}
            onPanel={setDocPanel}
            onMenu={setMenu}
            onModal={setModal}
          />
