...
export function WorkbenchChatStage({ onOpenDoc, onMenu }: { onOpenDoc: () => void; onMenu: (label: string) => void }) {
  return (
    <section className="wb-stage wb-chat-stage" aria-label="新版工作台对话区">
      <div className="wb-chat-scroll">
        ...
        <div className="wb-thinking">
          {chatSteps.map((step, index) => (
            <div key={`${step.kind}-${index}`} className={`wb-step is-${step.kind}`}>
        ...
        <article className="wb-result-card">
          ...
          {diffRows.map((row) => (
            <p key={row.no} className={row.kind === '+' ? 'is-add' : 'is-remove'}>
              <span>{row.no}</span>
