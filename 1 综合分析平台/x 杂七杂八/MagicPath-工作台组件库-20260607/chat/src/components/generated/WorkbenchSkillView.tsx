...
export function WorkbenchSkillView({ onMenu, onModal }: { onMenu: (label: string) => void; onModal: (label: string) => void }) {
  return (
    <section className="wb-stage wb-skill-view" aria-label="工作台技能">
      ...
      <div className="wb-skill-grid">
        {skills.map((card) => (
          <article key={card.id} className="wb-skill-card">
