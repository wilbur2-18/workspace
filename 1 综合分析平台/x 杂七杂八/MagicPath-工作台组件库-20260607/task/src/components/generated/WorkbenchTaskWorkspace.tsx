...
export function WorkbenchTaskWorkspace({
  ...
  return (
    <section className="wb-stage wb-task-stage" aria-label="工作台任务">
      <aside className="wb-batch-pane">
        ...
        <div className="wb-child-list">
          {batchChildren.map((child) => (
            <button key={child.id} type="button" className={`is-${child.status}`} onClick={onOpenDoc}>
      ...
      <div className={`wb-task-context ${detailOpen ? 'is-detail-open' : ''}`}>
        ...
        {detailOpen && (
          <aside className="wb-floating-detail">
            <header>
              <strong>{task.title}</strong>
