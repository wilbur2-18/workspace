...
const panels: Array<{ id: DocPanel; label: string; icon: string }> = [
  { id: 'file', label: '打开文件目录', icon: 'file' },
  { id: 'database', label: '打开库表目录', icon: 'table' },
  { id: 'graph', label: '打开数据图谱目录', icon: 'map' },
  { id: 'knowledge', label: '打开知识库目录', icon: 'book' },
  { id: 'result', label: '打开结果目录', icon: 'folder' },
];
...
export function WorkbenchDocWorkspace({
  ...
  return (
    <section className={`wb-doc ${fullscreen ? 'is-fullscreen' : ''}`} aria-label="资料预览与目录">
      <div className="wb-doc-tabs">
        <div className="wb-tab-strip">
          {docTabs.map((tab, index) => (
            <button key={tab.id} type="button" className={index === 0 ? 'is-active' : ''}>
              <Icon name={tab.icon} />
              <span>{tab.title}</span>
      ...
      <div className="wb-doc-body">
        ...
        <aside className="wb-directory">
          <div className="wb-directory-switcher">
            {panels.map((item) => (
              <button key={item.id} type="button" className={panel === item.id ? 'is-active' : ''} title={item.label} onClick={() => onPanel(item.id)}>
                <Icon name={item.icon} />
              </button>
            ))}
          </div>
          <div className="wb-directory-list">
            ...
            {resourcesByPanel[panel].map((item) => (
              <button key={item.id} type="button" onClick={() => onMenu(item.title)}>
