...
export function WorkbenchSearchView({ onView, onTask }: { onView: (view: MainView) => void; onTask: (id: string) => void }) {
  return (
    <section className="wb-stage wb-search-view" aria-label="工作台搜索">
        ...
        <div className="wb-search-columns">
          <section>
            ...
            {conversations.map((item) => (
              <button key={item.id} type="button" className="wb-wide-card" onClick={() => onView('chat')}>
                <Icon name="chat" />
          ...
          <section>
            ...
            {tasks.slice(0, 5).map((item) => (
              <button key={item.id} type="button" className="wb-wide-card" onClick={() => { onTask(item.id); onView('task'); }}>
                <Icon name={item.icon} />
